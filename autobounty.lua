                        ...
                        if (tonumber(game.Players.LocalPlayer.Data.Level.Value) - 250) < v.Data.Level.Value  then
                                if (getgenv().Setting.Skip.RaceV4 and not v.Character:FindFirstChild("RaceTransformed")) or not getgenv().Setting.Skip.RaceV4 then
                                    ...
                                    if getgenv().Setting.Chat.Enabled then
                                        if getgenv().Setting.Chat.Messenger and getgenv().checked ~= nil then
                                                print(player.Name .. " died!")
                                                local lqm = math.random(1, #getgenv().Setting.Chat.Messenger)
                                                local tn = getgenv().Setting.Chat.Messenger[lqm]
                                                local chatService = game:GetService("ReplicatedStorage"):FindFirstChild("DefaultChatSystemChatEvents")
                                                if chatService and chatService:FindFirstChild("SayMessageRequest") then
                                                    chatService.SayMessageRequest:FireServer(tn, "All")
                                                end
                                            end
                                        end                                   
                                    end
...
local initialBounty = game.Players.LocalPlayer.leaderstats["Bounty/Honor"].Value
function wEarn(targ)
    local currentBounty = game.Players.LocalPlayer.leaderstats["Bounty/Honor"].Value
    local bountyEarned = currentBounty - initialBounty
    local elapsedTime = GetElapsedTime(startTime)
    
    if getgenv().Setting.Webhook.Enabled and getgenv().killed ~= nil or lp.Character:FindFirstChild("Humanoid").Health > 0  then
        local data = {
            ["content"] = "",
            ["embeds"] = {
                {
                    ...
                    ["fields"] = {
                        ...
                        {
                            ["name"] = "Current Bounty: ",
                            ["value"] = "```"..tostring(currentBounty):reverse():gsub("%d%d%d", "%1,"):reverse():gsub("^,", "").."```",
                            ["inline"] = true,
                        }
                    },
                    ["thumbnail"] = {
