...
Notification.new("<Color=Yellow> Loading OrangeHub <Color=/>"):Display()
...
local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/oCloud123/iu/main/iu.lua')))()
local Window = OrionLib:MakeWindow({Name = "Orange Hub Blox Fruit", HidePremium = false,IntroText = "Orange Hub", SaveConfig = true, ConfigFolder = "Orange Hub"})

function Notify(h,i)
    OrionLib:MakeNotification({
        Name = "Orange Hub",
        Content = h,
        Image = "rbxassetid://16369967987",
        Time = i
    })
end

Notify("Welcome To Orange Hub",5)
...
I:AddLabel("Owner : VThuong")

I:AddLabel("Dev : oCloud & QuocNguyen")
...
SE:AddParagraph("Note","EN :  When you turn on fish farm, Farm Piraha, Terror,... and at that time you are sitting on the boat and the boat shakes, you just need to turn on Speed Boats and the boat will be fine.")
...
if game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("ChooseTeam") then
    local selectedTeam = getgenv().Team or "Pirates"
                ...
                if _G.AutoStoreFruit then
                    ...
                    local function StoreFruits()
                        pcall(function()
                            ...
                            local function StoreFruitInServer(Fruit)
                                if Fruit:IsA("Tool") and Fruit:FindFirstChild("Fruit") then
                                    local fruitType = Get_Fruit(Fruit.Name)
                                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StoreFruit", fruitType, Fruit)
                                end
                            end
                    
                            -- Kiểm tra và lưu trữ trái cây trong nhân vật
                            for _, Fruit in pairs(plrChar:GetChildren()) do
                                StoreFruitInServer(Fruit)
                            end
                    
                            -- Kiểm tra và lưu trữ trái cây trong túi đồ
                    ...
                    -- Gọi hàm khi nhân vật được tạo và theo dõi sự kiện để cập nhật
                    Player.CharacterAdded:Connect(StoreFruits)
                    StoreFruits()  -- Đảm bảo gọi hàm một lần khi script bắt đầu chạy
                end
            end
        end
    end,
